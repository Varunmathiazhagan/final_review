...
// Add Badge and Points Schemas after the existing schemas
const badgeSchema = new mongoose.Schema({
  ...
  category: String, // 'Mentor', 'Contributor', 'Networker', etc.
  rarity: {
    type: String,
...
const alumniPointsSchema = new mongoose.Schema({
  alumniId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'Alumni',
    required: true
  ...
  contributorPoints: { type: Number, default: 0 },
  networkerPoints: { type: Number, default: 0 },
  jobPoints: { type: Number, default: 0 }
});

// Create models for new schemas
const Badge = mongoose.model('Badge', badgeSchema);
const AlumniPoints = mongoose.model('AlumniPoints', alumniPointsSchema);
  ...
  try {
    ...
    if (badgesCount === 0) {
      await Badge.insertMany([
        {
          name: 'Newcomer',
          description: 'Joined the AlumniConnect platform',
          icon: 'star-outline',
          color: '#8BC34A',
          pointsRequired: 0,
          category: 'General',
        ...
        {
          name: 'Mentor Badge',
          description: 'Helped students by providing mentorship',
        ...
        {
          name: 'Job Provider',
          description: 'Posted job opportunities for students',
          icon: 'briefcase',
          color: '#FF9800',
          pointsRequired: 50,
        ...
        {
          name: 'Internship Guru',
          description: 'Posted multiple internship opportunities',
        ...
        },
        {
          ...
          description: 'Connected with many alumni and students',
        ...
        {
          name: 'Forum Expert',
          description: 'Provided valuable answers in the discussion forum',
          icon: 'forum',
          color: '#4CAF50',
          pointsRequired: 100,
          category: 'Contributor',
        ...
        {
          name: 'Career Guru',
          description: 'Posted over 10 jobs or internships',
          icon: 'medal',
          color: '#F44336',
          pointsRequired: 200,
          category: 'Contributor',
        ...
        },
        {
          ...
          description: 'Recognized for exceptional mentorship',
          icon: 'trophy',
          color: '#FFD700',
          pointsRequired: 500,
          category: 'Mentor',
...
app.post('/api/jobs', async (req, res) => {
  try {
    ...
    // Award points to the alumni who posted the job
    const pointsData = {
      points: 20,
    ...
    try {
      await axios.post(
        `http://localhost:3000/api/alumni/${postedBy.alumniId}/award-points`,
        pointsData
      );
    } catch (pointsErr) {
      console.warn('Failed to award points for job posting:', pointsErr.message);
      // Continue execution - don't block job posting if points award fails
    }
...
app.post('/api/internships', async (req, res) => {
  try {
    ...
    // Award points to the alumni who posted the internship
    const pointsData = {
      points: 15,
      action: `Posted internship: ${title}`,
      category: 'Job'
    };
    ...
    try {
      ...
      console.warn('Failed to award points for internship posting:', pointsErr.message);
    }
...
app.patch('/api/notifications/:notificationId/read', async (req, res) => {
  try {
    ...
    if (!notification) {
      console.log(`Notification not found: ${notificationId}`); // Debug log
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    const leaderboard = await AlumniPoints.find()
      .sort({ points: -1 })
      .limit(100)
      .populate({
        path: 'alumniId',
        select: 'name department passedOutYear company'
      });
...
// Get Alumni Points
app.get('/api/alumni/:alumniId/points', async (req, res) => {
  try {
    const { alumniId } = req.params;
    ...
    if (!alumniPoints) {
      ...
      const newcomerBadge = await Badge.findOne({ name: 'Newcomer' });
      if (newcomerBadge) {
        alumniPoints.badges.push({
          badgeId: newcomerBadge._id,
          dateEarned: new Date()
        });
        await alumniPoints.save();
      }
      ...
      alumniPoints = await AlumniPoints.findOne({ alumniId })
        .populate('badges.badgeId');
    }
    
    // Get alumni's rank
    const rank = await AlumniPoints.countDocuments({ points: { $gt: alumniPoints.points } }) + 1;
    
    res.status(200).json({
      ...alumniPoints.toObject(),
      rank
    });
  } catch (err) {
    console.error('Get alumni points error:', err.message, err.stack);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});

// Award points to alumni
app.post('/api/alumni/:alumniId/award-points', async (req, res) => {
  try {
    const { alumniId } = req.params;
    const { points, action, category } = req.body;
    
    if (!points || !action) {
      return res.status(400).json({ message: 'Points and action are required' });
    }
    
    // Find or create alumni points record
    let alumniPoints = await AlumniPoints.findOne({ alumniId });
    if (!alumniPoints) {
      alumniPoints = new AlumniPoints({ alumniId, points: 0 });
    }
    
    // Update points
    alumniPoints.points += points;
    
    // Update category-specific points if provided
    if (category) {
      switch (category) {
        case 'Mentor':
          alumniPoints.mentorPoints = (alumniPoints.mentorPoints || 0) + points;
          break;
        case 'Contributor':
          alumniPoints.contributorPoints = (alumniPoints.contributorPoints || 0) + points;
          break;
        case 'Networker':
          alumniPoints.networkerPoints = (alumniPoints.networkerPoints || 0) + points;
          break;
        case 'Job':
          alumniPoints.jobPoints = (alumniPoints.jobPoints || 0) + points;
          break;
      }
    }
    ...
    const now = new Date();
    if (alumniPoints.lastActivity) {
      const lastActivityDate = new Date(alumniPoints.lastActivity);
      const diffDays = Math.floor((now - lastActivityDate) / (1000 * 60 * 60 * 24));
      
      if (diffDays <= 1) {
        // Activity within 1 day - increase streak
        alumniPoints.streak += 1;
      } else if (diffDays > 2) {
        // More than 2 days - reset streak
        alumniPoints.streak = 1;
      }
      // If diffDays is exactly 2, we maintain the current streak
    } else {
      // First activity - start streak
      alumniPoints.streak = 1;
    }
    
    alumniPoints.lastActivity = now;
    
    // Add to points history
    alumniPoints.pointsHistory.push({
      points,
      action,
      timestamp: now
    });
    ...
    // Check for new badges based on updated points
    const eligibleBadges = await Badge.find({
      pointsRequired: { $lte: alumniPoints.points },
      _id: { $nin: alumniPoints.badges.map(b => b.badgeId) }
    });
    
    const newBadges = [];
    for (const badge of eligibleBadges) {
      // For category-specific badges, check category points
      let eligible = true;
      if (badge.category === 'Mentor' && alumniPoints.mentorPoints < badge.pointsRequired) {
        eligible = false;
      } else if (badge.category === 'Contributor' && alumniPoints.contributorPoints < badge.pointsRequired) {
        eligible = false;
      } else if (badge.category === 'Networker' && alumniPoints.networkerPoints < badge.pointsRequired) {
        eligible = false;
      }
    ...
    alumniPoints.level = Math.floor(alumniPoints.points / 100) + 1;
    await alumniPoints.save();
    
    res.status(200).json({
      message: 'Points awarded successfully',
      points: alumniPoints.points,
      newBadges
    });
  } catch (err) {
    console.error('Award points error:', err.message, err.stack);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
...
app.get('/api/badges', async (req, res) => {
  try {
    ...
    console.error('Get badges error:', err.message, err.stack);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
